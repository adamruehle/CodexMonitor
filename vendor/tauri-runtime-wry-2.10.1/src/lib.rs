...
#[cfg(target_os = "macos")]
fn should_forward_drag_drop_event_to_tauri(
  event: &WryDragDropEvent,
  native_file_drag_active: &mut bool,
) -> bool {
  match event {
    WryDragDropEvent::Enter { paths, .. } => {
      let has_paths = !paths.is_empty();
      *native_file_drag_active = has_paths;
      has_paths
    }
    WryDragDropEvent::Over { .. } => *native_file_drag_active,
    WryDragDropEvent::Drop { paths, .. } => {
      let has_paths = !paths.is_empty();
      let should_forward = *native_file_drag_active || has_paths;
      *native_file_drag_active = false;
      should_forward
    }
    WryDragDropEvent::Leave => {
      let should_forward = *native_file_drag_active;
      *native_file_drag_active = false;
      should_forward
    }
    _ => true,
...
mod drag_drop_tests {
  use super::should_forward_drag_drop_event_to_tauri;
  use wry::DragDropEvent as WryDragDropEvent;

  #[test]
  fn rejects_non_file_drag_sessions() {
    let mut native_file_drag_active = false;

    assert!(!should_forward_drag_drop_event_to_tauri(
      &WryDragDropEvent::Enter {
        paths: vec![],
        position: (0, 0),
      },
      &mut native_file_drag_active,
    ));
    assert!(!native_file_drag_active);
    assert!(!should_forward_drag_drop_event_to_tauri(
      &WryDragDropEvent::Over { position: (1, 1) },
      &mut native_file_drag_active,
    ));
    assert!(!should_forward_drag_drop_event_to_tauri(
  ...
  #[test]
  fn keeps_native_pipeline_for_file_drag_sessions() {
    let mut native_file_drag_active = false;

    assert!(should_forward_drag_drop_event_to_tauri(
      &WryDragDropEvent::Enter {
        paths: vec!["/tmp/file.png".into()],
        position: (0, 0),
      },
      &mut native_file_drag_active,
    ));
...
"#,
  ...
  if webview_attributes.drag_drop_handler_enabled {
    ...
    #[cfg(target_os = "macos")]
    let native_file_drag_active = Arc::new(Mutex::new(false));
    webview_builder = webview_builder.with_drag_drop_handler(move |event| {
      #[cfg(target_os = "macos")]
      {
        let mut native_file_drag_active = native_file_drag_active.lock().unwrap();
        if !should_forward_drag_drop_event_to_tauri(&event, &mut native_file_drag_active) {
          return false;
        }
      }
